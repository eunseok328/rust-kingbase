...
mod tests {
    ...
    fn accepts_numeric_types() {
        ...
        assert!(<BigDecimal as ToSql>::accepts(&Type::MYSQL_NUMERIC));
        assert!(<BigDecimal as FromSql<'_>>::accepts(&Type::MYSQL_NUMERIC));
        assert!(<BigDecimal as ToSql>::accepts(&Type::MYSQL_DECIMAL));
        assert!(<BigDecimal as FromSql<'_>>::accepts(&Type::MYSQL_DECIMAL));
    }
    ...
    fn decodes_known_values() {
        ...
        assert_eq!(
            BigDecimal::from_sql(&Type::MYSQL_NUMERIC, &[0, 1, 255, 255, 64, 0, 0, 4, 0, 12])
                .unwrap()
                .to_string(),
            "-0.0012"
        );
    ...
    fn round_trips_basic_values() {
        for value in [
            ...
            let mut out = BytesMut::new();
            value.to_sql(&Type::MYSQL_DECIMAL, &mut out).unwrap();
            let decoded = BigDecimal::from_sql(&Type::MYSQL_DECIMAL, &out).unwrap();
